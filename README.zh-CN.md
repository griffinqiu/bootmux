...
[tmux](https://github.com/tmux/tmux) 或 [Herdr](https://herdr.dev/) 中运行。
bootmux 是一个独立的 Rust 二进制文件：无需 Ruby，也不会把 tmux 藏在
Herdr pane 里运行。
...
- **一种项目格式，两个原生后端。** 一个项目会成为 tmux session 或 Herdr
  workspace；window 会成为 tmux window 或 Herdr tab，pane 则始终是所选
  终端多路复用器中真正的 pane。
...
- **经过测试的兼容性。** 三个具有代表性的 tmux 渲染结果会与源自
  tmuxinator 的 golden snapshot 逐字节比较；另一套独立的 19 文件矩阵会在
  两个后端上验证固定版本的 mux fixture。
...
`debug` 会验证并渲染后端计划，但不会创建项目拓扑。Herdr debug 不会连接或启动
server；tmux debug 为了读取 `base-index`、`pane-base-index` 和活动 session
状态，可能会启动 tmux server。
...
活动的 Herdr popup 优先于继承的 tmux 环境变量。如果 tmux 和 Herdr 确实存在
嵌套，而 bootmux 无法识别前台归属，它会拒绝猜测，并要求显式指定后端。

## 已经有 tmuxinator 或 mux 项目？

把文件留在原来的目录中，先在两个后端上分别进行预检：
...
| 能力 | tmux | Herdr |
|---|---|---|
| 原生项目容器 | Session | Workspace |
| Window 映射 | Window | Tab |
| Pane 命令和工作目录 | 支持 | 支持 |
| 命名 socket 和自定义 socket | 支持 | 支持 |
| tmux 预设 layout | 原生应用 | 转换为 BSP 计划 |
| 序列化 tmux layout | 原生应用 | 严格解析后再转换 |
| `synchronize` | 支持 | 拒绝 truthy（按真值判断为真）的值：没有等价的输入语义 |
| `tmux_options`、`tmux_command`、pane 边框字段 | 支持 | 警告后忽略 |
| 停止时的身份依据 | 从配置渲染 session/socket；没有所有权状态 | 持久化 endpoint/config/name/root 的精确所有权记录 |

[Herdr session restore](https://herdr.dev/docs/session-state/) 会恢复终端拓扑和
工作目录，但不会恢复任意子进程的状态。复用匹配的 workspace 时会运行
`on_project_restart`，不会重新运行每个 pane 命令。在依赖重启恢复或
`stop-all` 前，请先阅读[后端与生命周期（英文）](docs/backends.md)。

正常的 Herdr 启动会把一个项目映射为一个 workspace。`--append` 则会把 tab
添加到当前活动的 workspace，不会创建一个可以独立停止的项目。
