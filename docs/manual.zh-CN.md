...
- tmux 2.6 或更新版本、Herdr 0.7.5/protocol 17、19 或 20，或两者都安装
- `$SHELL` 和 `$EDITOR`
- 可选的 `fzf`
...
```sh
mise use -g rust
mise use -g cargo:bootmux@0.3.2
bootmux version
```

如果只想为某个项目固定 bootmux，请在项目中运行不带 `-g` 的
`mise use cargo:bootmux@0.3.2`。
...
请升级 Herdr 客户端/服务器组合，使两者都使用受支持的 protocol（17、19 或 20）
并满足最低版本要求。
